...
if(isset($_POST["Submit"])){
  ...
  $DocTarget ="../Photos/".basename($_FILES["TDoc"]["name"]);
  ...
  if(empty($VehicleNum)){
    ...
    $sql = "INSERT INTO theft(DateTime, Vehicle_No, Tdate,Place,Complaint_Number, Document, AddedBy)";
    $sql .= "VALUES(:DT,:vno,:tdate,:place,:cno,:TDoc,:admin)";  //Dummy values
    ...
    $stmt->bindValue(':TDoc',$TDoc);
    $stmt->bindValue(':admin',$AddedBy);

    $Execute=$stmt->execute();
    move_uploaded_file($_FILES["TDoc"]["tmp_name"],$DocTarget); //to upload image

    if($Execute){
      $_SESSION["SuccessMessage"]= "Theft Entry for id: ".$ConnectingDB->lastInsertId()." is Successful";
      Redirect_to('MyProfile.php?ID='.$ConnectingDB->lastInsertId());
...
<body>
  ...
  <header class="bg-dark text-white py-3">
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          <h1><i class="fas fa-edit" style="color: #27a9e1d7;"></i> Theft Data Entry</h1>
        </div>
      </div>
    </div>
  </header>
  ...
  <section class="container py-2 mb-4">
    <div class="row" >
      <div class="offset-lg-1 col-lg-10" style="min-height: 100px;">
        ...
        echo SuccessMessage();
        ?>
        <form action="theft.php" class="" method="POST" enctype="multipart/form-data">
          <div class="card bg-secondary text-light mb-2 mt-5">
            <div class="class-header">
              <h1>Add New Theft Entry</h1>
            </div>
            <div class="card-body bg-dark">
              ...
              <div class="form-group">
                <label for="Tdate"><span class="FieldInfo"> Theft Date: </span></label>
                <input type="date" class="form-control" name="Tdate" id="Tdate" placeholder="Type When Theft Occurred here">
              </div>
            
              <div class="form-group">
                <label for="Place"><span class="FieldInfo"> Theft Place: </span></label>
                <input type="text" class="form-control" name="Place" id="Place" placeholder="Type Where Theft Occurred here">
              </div>
              <div class="form-group">
                <label for="CNo"><span class="FieldInfo"> Complaint Number: </span></label>
                <input type="text" class="form-control" name="CNo" id="CNo" placeholder="Type Complaint Number here">
              </div>
              
              <div class="form-group  mb-4">
                <label for="TDoc"><span class="FieldInfo"> Choose FIR Document </span></label>
                <div class="custom-file">
                  <input class="custom-file-input" type="file" name="TDoc" id="TDoc" value="">
                  <label for="TDoc" class="custom-file-label"> Document Image</label>
                </div>
              </div>
              <div class="row">
