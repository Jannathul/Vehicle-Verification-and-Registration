...
if(isset($_POST["Submit"])){
  ...
  if(empty($UserName)||empty($Password)||empty($ConfirmPassword)){
    ...
    $sql .= "VALUES(:DT,:username,:password,:name,:admin)";  //Dummy values
    $stmt = $ConnectingDB->prepare($sql);
    $stmt->bindValue(':DT',$DateTime);
    $stmt->bindValue(':username',$UserName);
    $stmt->bindValue(':password',$Password);
    $stmt->bindValue(':name',$Aname);
...
<body>
  ...
  <header class="bg-dark text-white py-3">
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          <h1><i class="fas fa-user" style="color: #27a9e1d7;"></i> Manage Admin </h1>
        </div>
      </div>
    </div>
  </header>
  ...
  <section class="container py-2 mb-4">
    <div class="row" >
      <div class="offset-lg-1 col-lg-10" style="min-height: 100px;">
        ...
        echo SuccessMessage();
        ?>
        <form action="Admin.php" class="" method="POST" enctype="multipart/form-data">
          <div class="card bg-secondary mb-2 mt-5">
            <div class="class-header">
              <h1 class="text-light">Add New Admin</h1>
            </div>
            <div class="card-body bg-dark">
              ...
              <div class="form-group">
                <label for="aname"><span class="FieldInfo"> Admin Name: </span></label>
